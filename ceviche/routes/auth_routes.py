...
def login():
    try:
        ...
        print(f"[DEBUG] Username: {username}, Password presente: {bool(password)}, Role: {role}, Device ID: {device_id}, Force new: {force_new}")
        ...
        # Si no se envía el rol, tomar el rol del usuario desde la base de datos
        if not role:
            role = user.role

        # Crear sesión en el sistema mejorado
        session['user_id'] = user.id
        session['username'] = user.username
        session['role'] = role
        session['device_id'] = device_id
        print(f"[DEBUG] Sesión de Flask creada para usuario ID: {user.id}, rol: {role}, device_id: {device_id}")

        # Crear sesión en la base de datos (usando SessionService)
        SessionService.create_session(user, role=role, device_id=device_id, force_new=force_new)
