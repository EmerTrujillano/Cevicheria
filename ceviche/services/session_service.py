...
class SessionService:
    ...
    def create_session(user, force_new=False):
        ...
        try:
            ...
            @staticmethod
            def create_session(user, role=None, device_id=None, force_new=False):
                """
                Crear nueva sesión para un usuario por rol y dispositivo
                Args:
                    user: Objeto User
                    role: Rol del usuario (si aplica)
                ...
                try:
                    ...
                    # Buscar sesiones activas para este usuario, rol y dispositivo
                    ...
                    if existing_sessions:
                        print(f"[SESSION_SERVICE] Usuario {user.username} ya tiene {len(existing_sessions)} sesión(es) activa(s) para rol {role} y dispositivo {device_id}")
                        for session in existing_sessions:
                            session.invalidate('new_login_same_user_role_device')
                        db.session.commit()
                    ...
                    flask_session['user_id'] = user.id
                    flask_session['username'] = user.username
                    flask_session['role'] = role if role else user.role
                    flask_session['device_id'] = device_id
                    flask_session.permanent = True
                    return {
                        'success': True,
                        'session': new_session.to_dict(),
